...
import { api, downloadFile, type FileResource, type KnowledgeRef, type Project } from '../api/client'
...
async function download(file: FileResource) { try { await downloadFile('files', file.id, file.name) } catch (e) { message.value = e instanceof Error ? e.message : '下载失败' } }
