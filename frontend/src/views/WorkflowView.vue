...
const props = defineProps<{ project: Project | null; resources: WorkspaceResource[] }>()
...
async function load() { if (!props.project) return; panel.value = 'none'; message.value = ''; try { const [workflow, loadedFiles, loadedConnections, loadedPptSkills] = await Promise.all([api.getProjectWorkflow(props.project.id), api.listFiles(props.project.id), api.listConnections(props.project.id), api.listPptSkills()]); files.value = loadedFiles; connections.value = loadedConnections; pptSkills.value = loadedPptSkills; openWorkflow(workflow); emit('workflowChanged', workflow) } catch (error) { message.value = error instanceof Error ? error.message : '工作流没有加载' } }
async function save(silent = false) { if (!props.project) return null; busy.value = true; try { const saved = await api.saveProjectWorkflow(props.project.id, payload()); activeId.value = saved.id; currentVersion.value = saved.currentVersion; nextRunAt.value = saved.nextRunAt ?? ''; emit('workflowChanged', saved); if (!silent) message.value = saved.status === 'READY' ? `已保存第 ${saved.currentVersion} 版` : `草稿已保存为第 ${saved.currentVersion} 版，还有内容需要补充`; return saved } catch (error) { message.value = error instanceof Error ? error.message : '工作流没有保存'; return null } finally { busy.value = false } }
...
watch(() => props.project?.id, () => { activeId.value = ''; eventSource?.close(); liveRunId.value = ''; liveEvents.value = []; liveContent.value = ''; liveContentNode.value = ''; load() }); onMounted(() => { load(); window.addEventListener('keydown', handleDeleteKey) }); onBeforeUnmount(() => { window.clearTimeout(pollTimer); eventSource?.close(); window.removeEventListener('keydown', handleDeleteKey) })
</script>

<template>
  <main class="workflow-workspace" :class="{ 'has-notice': !!message }">
    <header class="workflow-header">
      <div><p class="eyebrow">主工作流 · 第 {{ currentVersion }} 版</p><input v-model="name" class="workflow-title-input" aria-label="工作流名称"><input v-model="description" class="workflow-description-input" aria-label="工作流说明"></div>
      <div class="workflow-actions"><button class="secondary-button" type="button" @click="panel = 'add'"><Plus :size="16"/>添加内容</button><button class="secondary-button" type="button" @click="autoLayout"><Network :size="16"/>自动排布</button><button class="secondary-button" type="button" @click="panel = 'schedule'"><CalendarClock :size="16"/>{{ executionMode === 'SCHEDULED' ? '定时执行' : '手工执行' }}</button><button v-if="latestRun || liveEvents.length" class="secondary-button" type="button" @click="panel = 'progress'"><Activity :size="16"/>运行进展</button><button class="secondary-button" type="button" @click="panel = 'history'; loadRuns()"><History :size="16"/>执行历史</button><button class="secondary-button" type="button" :disabled="busy" @click="save()"><Save :size="16"/>保存</button><button class="secondary-button icon-command" type="button" title="检查工作流" @click="check"><CheckCircle2 :size="17"/></button><button class="primary-button" type="button" :disabled="busy || runActive" @click="run"><Play :size="16"/>立即运行</button></div>
    </header>
