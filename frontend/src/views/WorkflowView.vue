...
import WorkflowNodeData from '../components/WorkflowNodeData.vue'
import { workProgress } from '../domain/workProgress'
...
const liveMessage = computed(() => { const event = [...liveEvents.value].reverse().find(event => event.message); return event ? workProgress(event).detail : statusText(latestRun.value?.status ?? 'QUEUED') })
...
const selectedAncestors = computed(() => {
  const found = new Set<string>(), queue = [selectedNodeId.value]
  while (queue.length) {
    const target = queue.shift()
    for (const edge of edges.value) if (edge.target === target && !found.has(edge.source)) { found.add(edge.source); queue.push(edge.source) }
  }
  return nodes.value.filter(node => found.has(node.id) && node.id !== selectedNodeId.value)
})
  ...
  <main class="workflow-workspace" :class="{ 'has-notice': !!message }">
    ...
    <section class="workflow-canvas" aria-label="工作流编排画布" @dragover.prevent @drop.prevent="onCanvasDrop">
      ...
      <aside v-if="selectedNode && panel === 'none'" class="canvas-drawer node-drawer"><header><div><span>步骤设置</span><input v-model="selectedNode.data.label" aria-label="步骤名称"></div><button class="icon-button danger" type="button" title="删除步骤" @click="removeSelected"><Trash2 :size="16"/></button></header><div class="form-stack compact-fields">
        <WorkflowNodeData :key="selectedNode.id" :node="selectedNode" :ancestors="selectedAncestors" :run="latestRun"/>
