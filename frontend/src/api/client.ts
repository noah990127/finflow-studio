...
export type WorkflowActivity = { id: string; runId: string; nodeRunId: string; order: number; type: 'PLAN' | 'TOOL_CALL' | 'SUBAGENT' | 'VALIDATION' | 'ARTIFACT' | 'MESSAGE'; capability: string; title: string; status: string; input: Record<string, unknown>; output: Record<string, unknown>; errorMessage: string; startedAt?: string; finishedAt?: string }
export type WorkflowLineageEdge = { id: string; runId: string; nodeRunId?: string; sourceKind: string; sourceRef: string; sourceVersion?: number; targetKind: string; targetRef: string; targetVersion?: number; relation: string; details: Record<string, unknown>; createdAt: string }
export type WorkflowNodeRun = { id: string; nodeId: string; nodeName: string; nodeType: WorkflowNodeType; stepOrder: number; status: string; input: Record<string, unknown>; output: Record<string, unknown>; errorMessage: string; activities: WorkflowActivity[]; startedAt?: string; finishedAt?: string }
export type WorkflowRun = { id: string; workflowId: string; projectId: string; workflowVersion: number; retryOfRunId?: string; triggerType: 'MANUAL' | 'SCHEDULED' | 'RETRY'; status: string; currentNodeId?: string; output: Record<string, unknown>; errorMessage: string; traceId: string; nodes: WorkflowNodeRun[]; lineage: WorkflowLineageEdge[]; createdAt: string; startedAt?: string; finishedAt?: string }
export type WorkflowProgressEvent = { sequence: number; runId: string; type: 'RUN_STATUS' | 'NODE_STARTED' | 'STEP_PROGRESS' | 'MODEL_STATUS' | 'MODEL_OUTPUT' | 'AGENT_TOOL_STARTED' | 'AGENT_TOOL_COMPLETED' | 'NODE_COMPLETED' | 'NODE_FAILED' | 'REVIEW_REQUIRED' | 'REVIEW_CONFIRMED' | 'RUN_COMPLETED'; nodeId: string; nodeName: string; status: string; progress: number; message: string; content: string; createdAt: string }
...
export type WorkflowPatch = { baseRevision: number; summary: string; operations: Array<Record<string, unknown>>; missingInputs: string[]; assumptions: string[]; expectedOutputs: string[] }
...
export type WorkflowSummary = { id: string; name: string; status: string; currentVersion: number; updatedAt: string }
export type ProjectWorkspace = { project: Project; workflow: WorkflowSummary | null; workflows: WorkflowSummary[]; folders: WorkspaceFolder[]; resources: WorkspaceResource[] }
...
export const api = {
  ...
  sendMessage: (sessionId: string, projectId: string, text: string, page = 'project-home', selection?: Selection, executionMode: 'AUTO' | 'APPROVAL' = 'APPROVAL', requestId?: string) => request<MessageResponse>(`/api/assistant/sessions/${sessionId}/messages`, { method: 'POST', body: JSON.stringify({ projectId, text, page, route: window.location.pathname, selection, clientContextVersion: 1, executionMode, requestId }) }),
  ...
  getWorkflowActivities: (id: string) => request<WorkflowActivity[]>(`/api/workflow-runs/${id}/activities`),
  getWorkflowLineage: (id: string) => request<WorkflowLineageEdge[]>(`/api/workflow-runs/${id}/lineage`),
  getSolidificationPatch: (id: string) => request<WorkflowPatch>(`/api/workflow-runs/${id}/solidification-patch`),
  applyWorkflowPatch: (id: string, patch: WorkflowPatch) => request<Workflow>(`/api/workflows/${id}/patches/apply`, { method: 'POST', body: JSON.stringify(patch) }),
