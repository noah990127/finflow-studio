...
导入器会同时恢复原始资料、加工结果、数据源、工作流和精选输出快照。快照包含交互报告、管理层 PPT、HTML 演示、Word 报告及案例支持的图形文件；导入过程不调用大模型，未配置模型密钥时也能直接查看完整案例。再次运行工作流时才会使用目标服务器配置的模型生成新版本。

导入器可重复执行：已有项目、文件和连接会复用，案例工作流会更新为仓库中的当前定义；只有快照文件内容发生变化时，才会为同名输出件导入新版本。
