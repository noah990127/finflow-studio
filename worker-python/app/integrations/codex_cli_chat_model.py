...
可以在一次响应中调用多个互不依赖的只读工具；有依赖关系或可能冲突的写操作必须等待上一步结果。看到工具结果后继续选择工具，任务规划完成后只返回
JSON：{{"final":{{"summary":"面向用户的简短说明","public_summary":"向用户简要说明目标理解、处理依据或根据真实结果所作的调整，不含内部推理","intent":"意图","selected_skills":["skill"],"completed":false}}}}。
只有根据真实工具结果确认目标完成时 completed 才能为 true。优先批量发起互不依赖的读取，减少无意义的模型往返。
